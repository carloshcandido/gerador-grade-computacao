...
export function gerarSidebar() {
    ...
    // Bloco de Créditos de Optativas/Eletivas
    const extraCreditsContainer = document.createElement('div');
    extraCreditsContainer.style.cssText = 'padding: 10px; margin-bottom: 15px; background-color: #e9ecef; border-radius: 5px; display: flex; flex-direction: column; gap: 5px;';
    extraCreditsContainer.innerHTML = `
    ...
    `;
    sidebarContent.appendChild(extraCreditsContainer);
    extraCreditsContainer.querySelector('input').addEventListener('input', updateAll);
...
export function atualizarEstadoPrerequisitos() {
    ...
    const valorOptativas = optativasInput ? parseInt(optativasInput.value) || 0 : 0;
    const totalParaPrereq = creditosConcluidos + valorOptativas;
...
export function atualizarContador() {
    ...
    const optativasInput = document.getElementById('optativas-concluidas');
    const valorOptativas = optativasInput ? parseInt(optativasInput.value) || 0 : 0;
    const totalConcluidoComOptativas = creditosConcluidos + valorOptativas;

    const creditosFaltantes = totalCreditosObrigatorios - creditosConcluidos;
    document.getElementById('contador-progresso').innerHTML = `Matérias Cursadas: <strong>${concluidas}</strong> | Faltam: <strong>${faltam}</strong><br>Créditos Concluídos: <strong>${totalConcluidoComOptativas}</strong> | Faltam: <strong>${creditosFaltantes}</strong>`;
}

export function exportarGradeParaPDF(gradeElement) {
    const botoes = gradeElement.querySelectorAll('.btn-acao');
    const checkboxesPrioridade = gradeElement.querySelectorAll('label'); // Esconde "Priorizar TCC" e "Priorizar Tempo"
    botoes.forEach(b => b.style.visibility = 'hidden');
    checkboxesPrioridade.forEach(l => l.style.visibility = 'hidden');
    ...
    html2pdf().from(gradeElement).set(opt).save().then(() => {
        ...
        checkboxesPrioridade.forEach(l => l.style.visibility = 'visible');
    });
}